...
const BlankGameSingle = () => {
  ...
  const slides = [
    {
      img: "/ox_image/guide1.png",
      desc: "제한 시간안에 빈칸에 알맞은 답을 입력하세요!!",
    },
    {
      img: "/ox_image/guide2.png",
      desc: "문제를 틀리면 외계인이 나타나 공격하니 주의하세요!!",
    },
    {
      img: "/ox_image/guide3.png",
          ...
          <div className="ox-charselect-list">
            {[1, 2, 3, 4, 5].map((num) => {
              ...
              return (
                <button
                  key={num}
                  className={`ox-charselect-btn ${colorClass}${
                    selectedChar === num ? " selected" : ""
                  }`}
                  onClick={() => handleCharSelect(num)}
                  value={num}
                >
                  <img
                    src={`/ox_image/char${num}.png`}
                    alt={`캐릭터${num}`}
...
export default BlankGameSingle;
